...
export function DishPage() {
  const { monthId, dayId } = useParams<{ monthId: string; dayId: string }>();
  console.log(
    "monthId",
    monthId,
    parseInt(monthId, 10),
  ...
  const month = monthId ? parseInt(monthId, 10) : null;
  ...
  const dish = italianDishes[month][day];
  ...
  const nextMonth = day === 31 ? (month === 11 ? 1 : month + 1) : month;
